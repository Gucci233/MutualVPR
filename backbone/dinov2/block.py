...
class MixVPR(nn.Module):
    def __init__(self,
        ...
        self.D_fc1 = nn.Linear(768, in_channels)
    def forward(self, x):
        x=self.D_fc1(x)
        x0 = F.relu(x, inplace=True)
        clstoken =  x0[:,0:1,:]
        x = x0[:,1:,:]
        x = x.permute(0, 2, 1)
        x = self.mix(x)
...
#     def __init__(self, fc_in_channels,in_channels=256):
#         super(MulConvAdapter, self).__init__()
...
#         x = x0[:,1:,:]
#         x = x.reshape(B,W,H,D).permute(0,3,1,2)
#         x = self.stn(x)
#         clstoken =  x0[:,0:1,:]
#         x = x.reshape(B,D,W*H).permute(0,2,1)
#         outputs = torch.cat([clstoken,x],dim=1)
...
class Block(nn.Module):
    def __init__(
        ...
        # self.adapter= MulConvAdapter(768,256)
